...
func (itp *Interpreter) advance(lastFrame int) (g.Value, g.Error) {
	...
	case bc.Invoke:
		...
		default:
			return nil, g.TypeMismatchError(g.FuncType)
		}
	...
	case bc.Throw:
		...
		if !ok {
			return nil, g.TypeMismatchError(g.StrType)
		}
	...
	case bc.SetField, bc.InitField, bc.InitProperty, bc.InitReadonlyProperty:
		...
		case bc.SetField:
			...
			if !ok {
				return nil, g.TypeMismatchError(g.StructType)
			}
			value := f.stack[n]
	...
	case bc.JumpTrue:
		...
		if !ok {
			return nil, g.TypeMismatchError(g.BoolType)
		}
	...
	case bc.Rem:
		...
		if !ok {
			return nil, g.TypeMismatchError(g.IntType)
		}
